...
pias <- ggplot(data = lstab, aes(x = factor(LUH), y = exp(Estimate)-1, colour = CT, shape = CT)) + 
  geom_point(data = lstab, aes(y = exp(Estimate)-1, fill = factor(LUH)), size = 10, show.legend = FALSE) +
  ...
  scale_fill_manual(values=c("darkolivegreen", "gold2")) +
...
pw1 <- ggplot(data.cast.whc, 
  ...
  geom_point(alpha = .15, show.legend=F) + labs(colour = "Legend", y = NULL,x = "Soil water holding capacity (%)") +
  ...
  theme(panel.background = element_rect(fill = NA, color = "black"), panel.grid = element_blank(),
        ...
        axis.text.y = element_text(size = 10, color = "black"), legend.position=c(.5,.9))
...
pl2 <- ggplot(ear.light, 
  ...
  theme(panel.background = element_rect(fill = NA, color = "black"), panel.grid = element_blank(),
        ...
        legend.position=c(.5,.9), legend.box = "horizontal")
